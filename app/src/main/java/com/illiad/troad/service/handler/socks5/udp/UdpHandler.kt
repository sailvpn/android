...
class UdpHandler(private val ctx: ChannelHandlerContext) {

    suspend fun setupUdpConnection(connection: Connection) = withContext(Dispatchers.IO) {
        val session = Demux.getSession(connection) ?: return@withContext
        if (session.isConnecting) return@withContext
        session.isConnecting = true

        Bootstrap().group(ctx.channel().eventLoop()).channel(NioSocketChannel::class.java)
